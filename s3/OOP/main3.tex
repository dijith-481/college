...
\begin{itemize}
    ...
    \item \textbf{Lack of a Single Source of Truth:} When data logic is scattered across multiple client applications, there is no centralized point of control for data access rules, validation, or transformation. 
...
\begin{itemize}
  ...
  \item \textbf{Brittleness and High Maintenance Overhead:} Client applications often contain logic that is explicitly tied to the database schema. If a database administrator renames a column, every client application that references the old column name will break. This necessitates a coordinated, and often costly, update and redeployment of all dependent applications.
    \item \textbf{On-the-Fly Data Transformation:} The API can deliver data in a variety of formats (JSON, CSV, XML, etc.) based on a simple URL parameter.  enabling clients to receive data in the exact structure they require without any backend code changes.
    \item \textbf{Centralized Security and Control:} The API enforces a Role-Based Access Control (RBAC) model using API keys. Permissions (read-only, write, admin) are managed centrally, providing a single, secure point of access to the data.
\end{itemize}
This approach establishes a single centralized source of truth for data access, significantly reduces the client-side development effort, and creates a more secure and maintainable system architecture.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{diagrams/activity1.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{diagrams/activity2.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{dfd00.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{dfd01.png}
...
\begin{itemize}
    ...
    \item \textbf{NeoVim:}TextEditor 
    \item \textbf{JDTLS:} Language server
\end{itemize}
...
\begin{description}
   ...
   \item[\texttt{replace=\textless{}db\textunderscore{}field\textgreater{}:\textless{}alias\textgreater{}}] Performs a temporary, one-off renaming of a field.
\end{description}
...
\noindent\textbf{Steps:}
\begin{enumerate}
    \item A mock HTTP \lstinline|GET| request is simulated and passed to the DataRequest Handler.
...
\noindent\textbf{Precondition:} The entire system is compiled, running, and accessible at \lstinline|http://localhost:8080|.
...
\begin{enumerate}
    ...
    \item Using \lstinline|curl| and the \textbf{new write-access key}, send a \lstinline|POST| request to /data/inventory to add a new item.
...
The API has significant potential for future enhancements and expansions. The key areas for future development are:

\begin{itemize}
    \item \textbf{Relational Data Support (JOINs)}: Implementing support for relational \texttt{JOIN} operations within the URL-based query language. This will allow clients to efficiently fetch related data from multiple tables in a single API call, which is a critical feature for complex applications.

    \item \textbf{Performance Caching}: Introducing a server-side **caching layer** to store the results of frequent, identical read queries. This would dramatically reduce the load on the underlying database and provide near-instantaneous response times for commonly accessed data.

    \item \textbf{Containerized Deployment (Docker)}: Creating an official **Docker image** for the application. This will encapsulate the server into a single, portable container, allowing for one-command deployment and ensuring a consistent, reproducible environment for both development and production.

    \item \textbf{Administrative GUI}: Developing a lightweight, web-based **graphical user interface (GUI)** for administrators. This would provide a user-friendly alternative to the API for managing system configurations like API keys and views, making the system more accessible to a wider audience.

    \item \textbf{Expanded Database Support}: While the system is designed to be database-agnostic, future work will include official testing, optimization, and documentation for integration with major enterprise-grade databases such as \textbf{PostgreSQL} and \textbf{MySQL}.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% --- REFERENCES ---
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
M. Fowler, "Richardson Maturity Model: steps toward the glory of REST," \textit{martinfowler.com}, 2010. [Online]. 
